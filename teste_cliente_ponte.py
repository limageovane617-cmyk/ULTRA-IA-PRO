...
Valida:
1. Leitura segura do segredo de ambiente (sem nunca expÃ´-lo).
2. Envio de arquivo Python via POST /api/ponte/v2/processar com header x-api-secret.
3. TransformaÃ§Ã£o do cÃ³digo e validaÃ§Ã£o de sintaxe.
4. ExecuÃ§Ã£o real no interpretador Python e verificaÃ§Ã£o de exitCode 0.
5. Download automÃ¡tico do arquivo processado atravÃ©s da URL retornada.
6. GravaÃ§Ã£o local e verificaÃ§Ã£o de integridade do arquivo recebido.
"""
...
def executar_teste_automatizado():
    print("=" * 65)
    print("ðŸ§ª INICIANDO TESTE AUTOMATIZADO: cliente_ponte_alex.py")
    print("=" * 65)

    # 1. Verifica se a variÃ¡vel PONTE_API_SECRET estÃ¡ configurada
    segredo_presente = bool(os.environ.get("PONTE_API_SECRET", "").strip())
    if not segredo_presente:
        print("âŒ ERRO: PONTE_API_SECRET nÃ£o estÃ¡ definida no ambiente.")
        sys.exit(1)
    ...
    # 3. Teste de Ping
    print("\nðŸ” 3. Testando Health Check (GET /api/ponte/v2/ping)...")
    try:
        dados_ping = cliente.ping()
        print(f"   Status: {dados_ping.get('status')} | VersÃ£o: {dados_ping.get('version')} | Runtime: {dados_ping.get('pythonRuntime')}")
        assert dados_ping.get("status") == "online", "Status do ping deve ser 'online'"
        print("   âœ… Ping concluÃ­do com HTTP 200 OK.")
    except Exception as e:
        print(f"   âŒ Falha no ping: {e}")
        sys.exit(1)

    # 4. Cria arquivo temporÃ¡rio de entrada
    ...
    print(f"\nðŸ“ 4. Arquivo de teste gerado: {arquivo_entrada.name} ({arquivo_entrada.stat().st_size} bytes)")
    ...
    print(f"\nðŸš€ 5. Enviando requisiÃ§Ã£o para POST /api/ponte/v2/processar...")
    print(f"   InstruÃ§Ã£o: {instrucao_transformacao}")
    ...
    try:
        ...
        print("\nðŸ“¥ 6. Resposta recebida da Ponte Alex v2:")
        print(f"   â€¢ HTTP Status: {resultado['status_http']}")
        print(f"   â€¢ Test Passed: {resultado['test_passed']}")
        print(f"   â€¢ Mensagem: {resultado['test_message']}")
        print(f"   â€¢ Exit Code Python: {resultado['execution'].get('exitCode')}")
        print(f"   â€¢ Stdout: {resultado['execution'].get('stdout', '').strip()}")
        print(f"   â€¢ Download URL: {resultado['download_url']}")
        print(f"   â€¢ Arquivo Salvo Localmente: {resultado['arquivo_salvo']}")
        print(f"   â€¢ Tamanho Salvo: {resultado['tamanho_bytes']} bytes")

        # 7. ValidaÃ§Ãµes de AsserÃ§Ã£o
        ...
        # 8. Executa localmente o arquivo baixado para confirmaÃ§Ã£o extra
        conteudo_baixado = arquivo_saida_esperada.read_text(encoding="utf-8")
        assert "TESTE_INTEGRACAO_CLIENTE_SUCESSO" in conteudo_baixado, "O arquivo salvo deve conter o cÃ³digo alterado"

        proc_local = subprocess.run([sys.executable, str(arquivo_saida_esperada)], capture_output=True, text=True, timeout=5)
        assert proc_local.returncode == 0, "ExecuÃ§Ã£o local do script baixado deve retornar cÃ³digo 0"
        assert "TESTE_INTEGRACAO_CLIENTE_SUCESSO" in proc_local.stdout, "SaÃ­da da execuÃ§Ã£o local deve conter a string alterada"

        print(f"\nðŸŽ‰ 7. VerificaÃ§Ã£o e ExecuÃ§Ã£o Local do Arquivo Baixado: SUCESSO!")
        print(f"   ExecuÃ§Ã£o Local: Exit Code {proc_local.returncode} | SaÃ­da: '{proc_local.stdout.strip()}'")

    except ErroPonteAlex as e:
        print(f"\nâŒ FALHA NO TESTE DA PONTE ALEX: {e}")
        if e.detalhes:
            print(f"Detalhes do erro: {e.detalhes}")
        sys.exit(1)
    except AssertionError as e:
        print(f"\nâŒ FALHA DE ASSERÃ‡ÃƒO: {e}")
        sys.exit(1)
    finally:
    ...
    print("\n" + "=" * 65)
    print("ðŸ† TODOS OS TESTES DO CLIENTE OFICIAL FORAM APROVADOS COM SUCESSO!")
    print("=" * 65)
